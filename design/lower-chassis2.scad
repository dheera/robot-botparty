...
/*
translate([-8.5+2,batt_w/2+15,10])
rotate([90,0,0])
cylinder(d=35);
translate([-8.5+50,batt_w/2+15,10])
rotate([90,0,0])
cylinder(d=35);
*/
...
*/
    ...
    translate([batt_l/2+0.5+5+1.5*t,0,0])
    cube_center([15,batt_w+2*t,6]);

difference() {
    cube_center([batt_l+2*t,batt_w+2*t,batt_h + t]);
    
    translate([0.5,0,t])
    cube_center([batt_l+1,batt_w,batt_h * 2]);
    ...
    translate([15,100,2+9-8/2])
    cube_center([15,100,8]);
